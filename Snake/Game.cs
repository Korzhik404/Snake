...
namespace Snake
{
    class Game
    {
        ...
        {
            Random rand = new Random(); // функцию случайной генерации с обьектом rand
            for (int i = 0; i < n; i++) // от рисовать границы поля
            {
                for (int j = 0; j < m; j++)
                {
                    map[i, j] = " ";
            ...
            yz = n - 2; xz = 1;
            map[yz, xz] = "X"; // поставить на поле «голову» змеи
            while (true) // и создать в нем «пищу»
            {
                xv = rand.Next(1, m - 2);
                yv = rand.Next(1, n - 2);
